...
function ResetPassword() {
    ...
    const onSubmit = form.handleSubmit(async (values: ResetPasswordType) => {
        try {
            ...
            const res = await axios.post(`${URL_API}/api/user/reset-password`, password, {
        ...
        <>
            <Card className='w-[400px]'>
                <CardHeader>
                    ...
                    <CardContent>
                        <Form {...form}>
                            <form className='space-y-4 p-4 flex flex-col' onSubmit={onSubmit}>
                                <FormField
                                    name="newPassword"
                                ...
                                />
                                <Button className="bg-yellow-500 text-black max-w-sm hover:bg-yellow-300" type="submit">
                                    Cambiar contraseña
                                </Button>
                            </form>
                        </Form>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button onClick={()=> setDialogOpen(false)} className="bg-yellow-500 text-black">
                            <Link href="/login">Aceptar</Link>
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
